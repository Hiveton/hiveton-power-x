...
fail() {
  printf 'FAIL real-device-closure-failure-report-test: %s\n' "$1" >&2
  exit 1
}
...
printf 'real-device-closure-failure-firmware' >"$firmware"
...
[[ -s "$tmp_root/artifacts/goal/real-device-closure.md" ]] \
  || fail "closure did not write blocked status report"

closure_report="$tmp_root/artifacts/goal/real-device-closure.md"
grep -Fq -- '- Overall status: `blocked`' "$closure_report" \
  || fail "closure report does not mark blocked status"
grep -Fq "Firmware SHA256: \`$sha\`" "$closure_report" \
  || fail "closure report does not include current firmware SHA"
grep -Fq '| WCH ISP flash | blocked |' "$closure_report" \
  || fail "closure report does not mark WCH ISP flash blocked"
grep -Fq 'artifacts/flash/latest.md' "$closure_report" \
  || fail "closure report does not point to latest flash attempt"
grep -Fq 'artifacts/hardware-validation/latest.md' "$closure_report" \
  || fail "closure report does not include requested hardware report path"
grep -Fq 'no-isp-device' "$tmp_root/artifacts/flash/latest.md" \
  || fail "fake flash attempt did not leave no-ISP report"

printf '%s\n' "PASS real-device-closure-failure-report-test"
