...
fail() {
  printf 'FAIL real-device-closure-stale-success-report-test: %s\n' "$1" >&2
  exit 1
}
...
grep -Fq "Flash success SHA256: \`$old_sha\`" "$closure_report" \
  || fail "closure report does not include stale success SHA"
grep -Fq -- '- Flash success status: `stale`' "$closure_report" \
  || fail "closure report did not mark stale flash success evidence"
grep -Fq -- '- Latest flash attempt status: `no-isp-device`' "$closure_report" \
  || fail "closure report does not include latest failed attempt status"

printf '%s\n' "PASS real-device-closure-stale-success-report-test"
