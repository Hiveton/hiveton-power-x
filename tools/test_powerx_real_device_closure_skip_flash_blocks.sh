...
fail() {
  printf 'FAIL real-device-closure-skip-flash-blocks-test: %s\n' "$1" >&2
  exit 1
}
...
printf 'real-device-closure-skip-flash-firmware' >"$firmware"
...
grep -Fq 'Flash was explicitly skipped' "$closure_report" \
  || fail "closure report does not explain skipped flash"

printf '%s\n' "PASS real-device-closure-skip-flash-blocks-test"
