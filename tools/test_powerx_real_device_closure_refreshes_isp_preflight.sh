...
fail() {
  printf 'FAIL real-device-closure-refreshes-isp-preflight-test: %s\n' "$1" >&2
  exit 1
}
...
printf 'real-device-closure-refresh-preflight-firmware' >"$firmware"
sha="$(shasum -a 256 "$firmware" | awk '{ print $1 }')"
report="$tmp_root/artifacts/hardware-validation/latest.md"

cat >"$tmp_root/artifacts/flash/isp-preflight.md" <<'REPORT'
# PX1 WCH ISP Preflight

- Time: 2026-05-19 07:00:00 +0800
- Status: `visible`
- Message: stale visible report
REPORT
...
- Status: `no-isp-device`
- Message: refreshed no ISP report
REPORT
printf 'fake refreshed preflight\n'
exit 2
SCRIPT
...
grep -Fq -- '- ISP preflight status: `no-isp-device`' "$closure_report" \
  || fail "closure report did not refresh stale ISP preflight status"
grep -Fq 'refreshed no ISP report' "$tmp_root/artifacts/flash/isp-preflight.md" \
  || fail "ISP preflight report was not refreshed after flash failure"

printf '%s\n' "PASS real-device-closure-refreshes-isp-preflight-test"
