...
fail() {
  printf 'FAIL real-device-closure-reports-isp-diagnosis-test: %s\n' "$1" >&2
  exit 1
}
...
printf 'real-device-closure-isp-diagnosis-firmware' >"$firmware"
...
grep -Fq -- "- ISP preflight diagnosis: $diagnosis" "$closure_report" \
  || fail "closure report does not include ISP preflight diagnosis"

printf '%s\n' "PASS real-device-closure-reports-isp-diagnosis-test"
